...
export function BoardPanel({
  ...
  return (
    <div className={`panel-stack market-pane ${isCompact ? "compact-market-pane" : ""}`} data-testid="board-panel">
      {isCompact ? (
        <SectionCard
          title="Заявки рядом"
          subtitle="Сначала короткий поток дел. Подробности открываются только когда нужны."
          actions={
            <div className="compact-toolbar">
              ...
              {bootstrap.capabilities.can_post_jobs ? (
                <button className="btn btn-primary" type="button" onClick={() => setComposerOpen((value) => !value)}>
                  {composerOpen ? "Свернуть" : "Новая заявка"}
                </button>
              ) : null}
            </div>
          }
      ...
      {bootstrap.capabilities.can_post_jobs && composerOpen && isCompact ? (
        <SectionCard
          title="Новая заявка"
          subtitle="Четыре поля, остальное можно добавить позже."
          actions={
            <div className="tag-row mobile-suggestion-row">
              {BOARD_EXAMPLES.map((example) => (
                <button
                  key={example.title}
                  className="tag suggestion-chip"
          ...
          >
            ...
            <div className="row-grid">
              ...
              <input
                ...
                placeholder="Бюджет от"
                value={formState.budget_from}
                onChange={(event) => setFormState((state) => ({ ...state, budget_from: event.target.value }))}
              />
            </div>
            <div className="segmented compact-segmented">
              {[
      ...
      <SectionCard
        ...
        actions={
          <div className={`segmented ${isCompact ? "compact-segmented" : ""}`}>
            {[
              { id: "all" as const, label: "Все" },
              { id: "own" as const, label: "Мои" },
            ].map((item) => (
        ...
        {posts.length ? (
          posts.map((post: JobPost) => (
            <article key={post.id} className="glass-card board-card market-card" data-testid="board-post-card">
              ...
              <h4>{post.title}</h4>
              <p className={isCompact ? "card-copy card-copy-clamped" : "card-copy"}>{post.description}</p>
              ...
              <div className="action-row">
                {post.is_owner ? (
                  post.response_count ? (
                    <button className="btn btn-primary" onClick={() => onOpenResponses(post.id)}>
                      {isCompact ? "Отклики" : "Посмотреть отклики"}
                    </button>
                  ) : (
                ...
                {post.can_respond ? (
                  <button className="btn" onClick={() => setResponsePostId(post.id)}>
                    {responsePostId === post.id ? "Свернуть" : isCompact ? "Отклик" : "Откликнуться"}
                  </button>
                ) : null}
              </div>
...
export function NetworkPanel({
  ...
  return (
    <div className={`panel-stack market-pane ${isCompact ? "compact-market-pane" : ""}`} data-testid="network-panel">
      {!isCompact ? (
        <section className="market-overview section-card">
          <div className="market-overview-copy">
      ...
      <SectionCard title={isCompact ? "Найти мастера" : "Подобрать мастера"} subtitle={isCompact ? "Один поиск и короткий фильтр по занятости." : "Пара фильтров вместо тяжёлого каталога анкет."}>
        {isCompact && bootstrap.capabilities.can_publish_master_profile ? (
          <div className="action-row compact-actions">
        ...
        {mastersQuery.data?.items.length ? (
          mastersQuery.data.items.map((master: MasterCard) => (
            <article key={master.external_user_id} className="glass-card master-card market-card" data-testid="network-master-card">
              ...
              {master.bio ? <p className={isCompact ? "card-copy card-copy-clamped" : "card-copy"}>{master.bio}</p> : null}
              <div className="meta-cloud">
              ...
              <div className="tag-row">
                {master.skills.slice(0, isCompact ? 2 : 4).map((skill) => (
              ...
              <div className="action-row">
                <button className="btn btn-primary" onClick={() => onOpenMaster(master.external_user_id)}>
                  {isCompact ? "Подробнее" : "Открыть страницу"}
                </button>
              </div>
            </article>
          ))
