...
def order_cancel_reasons(order_id: int, reasons: list[dict[str, str]]) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for reason in reasons:
        kb.row(
            InlineKeyboardButton(
                text=fit_button_text(reason["label"], max_len=34),
                callback_data=f"order_cancel_reason:{order_id}:{reason['code']}",
            )
        )
    add_back_row(kb, "Заказ", f"order_view:{order_id}")
    return kb.as_markup()
